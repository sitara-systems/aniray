...
class ExampleAnirayCinderBasic : public ci::app::App {
  ...
  static std::array<std::uint8_t, 3> colorToOutput(ci::Color color) {
    return std::array<std::uint8_t, 3>({static_cast<std::uint8_t>(color[0]),
  ...
  using NodeArrayOutputOLA =
      aniray::NodeArrayOutputOLA<NodeArray, colorToOutput>;
...
void ExampleAnirayCinderBasic::prepareSettings(
...
void ExampleAnirayCinderBasic::resetCamera() {
  mCam.lookAt(glm::vec3(center[0] + 0, center[1] / 5 + 18, center[2] + 12),
              center);
}
...
void ExampleAnirayCinderBasic::setup() {
  ...
  mNodeArraySamples =
      std::make_unique<NodeArray>(*mNodeArray, true, SAMPLE_GRID_SPACING);

  // Output the physical Nodes via OLA
  if (ENABLE_OUTPUT) {
    mNodeArrayOutput = std::make_unique<NodeArrayOutputOLA>(
        *mNodeArray, OUTPUT_OLA_INTERVAL_US);
  }

  // Use the virtual Nodes to inform physical Nodes
  mNodeArraySampler =
      std::make_unique<NodeArraySampler>(*mNodeArray, *mNodeArraySamples);

  // A basic rotation animation
  mRotationAnimation = std::make_unique<RotationAnimation<NodeArray>>(
      *mNodeArraySamples, mNodeArray->center(), 2.5, 1.5, 4000, 0, 3000);
  ...
  auto sphere =
      geom::Sphere().subdivisions(SPHERE_SUBDIVISIONS).radius(SPHERE_RADIUS);
  mSphere = gl::Batch::create(sphere, shader);

  // Find the center and focus the camera on the center
  center = {mNodeArray->center().x(), mNodeArray->center().y(),
            mNodeArray->center().z()};
...
void ExampleAnirayCinderBasic::draw() {
  ...
  // Sample the virtual Nodes, average the squares their RGB values, and assign
  // to the physical Nodes
  mNodeArraySampler->sample(
      [](std::shared_ptr<Node> const targetNode,
  ...
  if (mDrawLEDs) {
    for (std::shared_ptr<Node> node : mNodeArray->nodes()) {
      glm::vec3 offset(node->coords().x(), node->coords().y(),
                       node->coords().z());
      ...
      if (node->ignore()) {
        gl::color(ColorA(CM_RGB, 0, 0, 0, 1));
      } else {
...
CINDER_APP(ExampleAnirayCinderBasic, ci::app::RendererGl,
           ExampleAnirayCinderBasic::prepareSettings);

...
IOInterfaceModbus::IOInterfaceModbus(std::string tcpAddress,
                                     std::uint16_t tcpPort) {
  mInitializeOptions = {.mode = InitializeOptionsMode::TCP,
                        .tcpAddress = std::move(tcpAddress),
                        .tcpPort = tcpPort};
  initialize();

  // Refresh inputs to initially populate values and confirm all addresses are
  // reachable refreshInputs();
}
...
void IOInterfaceModbus::initialize() {
  ...
  case InitializeOptionsMode::TCP:
    initConnectionTCP(mInitializeOptions.tcpAddress,
                      mInitializeOptions.tcpPort);
    break;
  default:
    throw std::runtime_error(
        "IOInterfaceModbus: Unknown initialize options mode!");
    break;
  }
}

void IOInterfaceModbus::initConnectionTCP(std::string tcpAddress,
                                          std::uint16_t tcpPort) {
  const std::unique_lock<std::shared_mutex> lock(mMutexModbusInitialize);
  ...
  if (mCTX == nullptr) {
    mModbusInitialized = false;
    throw std::runtime_error(
        "IOInterfaceModbus: Unable to allocate libmodbus context");
  }
  if (modbus_connect(mCTX) == -1) {
    mModbusInitialized = false;
    modbus_free(mCTX);
    throw std::runtime_error("IOInterfaceModbus: Connection failed: " +
                             std::string(modbus_strerror(errno)));
  }
  mModbusInitialized = true;
  BOOST_LOG_TRIVIAL(info) << "IOInterfaceModbus: Connected to " << tcpAddress
                          << ":" << tcpPort;
}

// NOTE: This method relies on lock from calling function!
void IOInterfaceModbus::setupInputDiscreteNoLock(
    const std::string &name, std::uint8_t slaveID, std::uint8_t functionCode,
    ConfigFunctionsAddressLayout addressLayout, std::uint16_t startAddress,
  ...
  mInputsDiscreteModbus[name] = {
      ...
      .startAddress = static_cast<std::uint16_t>(
          startAddress - 1U), // seems to always be off by one (starts at 0?)
      .numAddressedItems = numAddressedItems,
      .enableClear = false};
}
void IOInterfaceModbus::setupInputDiscrete(
    const std::string &name, std::uint8_t slaveID, std::uint8_t functionCode,
    ConfigFunctionsAddressLayout addressLayout, std::uint16_t startAddress,
    std::uint16_t numAddressedItems) {
  ...
  setupInputDiscreteNoLock(name, slaveID, functionCode, addressLayout,
                           startAddress, numAddressedItems);
}
void IOInterfaceModbus::setupInputDiscrete(
    const std::string &name, std::uint8_t slaveID, std::uint8_t functionCode,
    ConfigFunctionsAddressLayout addressLayout, std::uint16_t startAddress,
    std::uint16_t numAddressedItems, std::uint8_t clearFunctionCode,
    std::uint16_t clearStartAddress, bool onlyClearSingleAddress,
    bool clearHigh) {
  const std::unique_lock<std::shared_mutex> lock(mMutexInputsDiscreteModbus);
  // Use below instead of initializer so that mutex lock covers
  setupInputDiscreteNoLock(name, slaveID, functionCode, addressLayout,
  ...
  mInputsDiscreteModbus[name].clearStartAddress =
      static_cast<std::uint16_t>(clearStartAddress - 1U);
...
void IOInterfaceModbus::refreshInputs() {
  const std::shared_lock<std::shared_mutex> initializeLock(
      mMutexModbusInitialize);
  const std::shared_lock<std::shared_mutex> inputsDiscreteLock(
      mMutexInputsDiscreteModbus);
  if (mModbusInitialized) {
    for (auto const &[name, configInputDiscrete] : mInputsDiscreteModbus) {
      updateInputDiscrete(configInputDiscrete);
    }
    // inputsDiscreteLock.unlock(); // will need this when other types of input
    // are added
  }
}

// NOTE: This method relies on lock from calling function!
void IOInterfaceModbus::updateInputDiscreteModbusRead(
  ...
  case FUNCTION_CODE_READ_BITS:
    res_read =
        modbus_read_bits(mCTX, configInputDiscrete.startAddress,
                         configInputDiscrete.numAddressedItems, &dest8[0]);
    break;

  case FUNCTION_CODE_READ_INPUT_BITS:
    res_read = modbus_read_input_bits(mCTX, configInputDiscrete.startAddress,
                                      configInputDiscrete.numAddressedItems,
                                      &dest8[0]);
    break;

  case FUNCTION_CODE_READ_REGISTERS:
    res_read = modbus_read_registers(mCTX, configInputDiscrete.startAddress,
                                     configInputDiscrete.numAddressedItems,
                                     &dest16[0]);
    break;

  case FUNCTION_CODE_READ_INPUT_REGISTERS:
    res_read = modbus_read_input_registers(
        mCTX, configInputDiscrete.startAddress,
        configInputDiscrete.numAddressedItems, &dest16[0]);
    break;

  default:
    throw std::runtime_error(
        "IOInterfaceModbus: Incorrect discrete input function code!");
    break;
  }
  if (res_read == -1) {
    throw std::runtime_error(
        "IOInterfaceModbus: Error reading discrete values: " +
        std::string(modbus_strerror(errno)));
  }
}

// NOTE: This method relies on lock from calling function!
void IOInterfaceModbus::updateInputDiscreteModbusClear(
    const ConfigInputDiscrete &configInputDiscrete) {
  if (configInputDiscrete.enableClear) {
    ...
    case FUNCTION_CODE_FORCE_SINGLE_COIL:
      res_clear = modbus_write_bit(mCTX, configInputDiscrete.clearStartAddress,
                                   clear8_value);
      break;

    case FUNCTION_CODE_FORCE_MULTIPLE_COILS:
      res_clear = modbus_write_bits(mCTX, configInputDiscrete.clearStartAddress,
                                    numClear, &clear8[0]);
      break;

    case FUNCTION_CODE_PRESET_SINGLE_REGISTERS:
      res_clear = modbus_write_register(
          mCTX, configInputDiscrete.clearStartAddress, clear16_value);
      break;

    case FUNCTION_CODE_PRESET_MULTIPLE_REGISTERS:
      res_clear = modbus_write_registers(
          mCTX, configInputDiscrete.clearStartAddress, numClear, &clear16[0]);
      break;

    default:
      throw std::runtime_error(
          "IOInterfaceModbus: Incorrect discrete input clear function code!");
      break;
    }
    if (res_clear == -1) {
      throw std::runtime_error(
          "IOInterfaceModbus: Error clearing discrete values: " +
          std::string(modbus_strerror(errno)));
    }
  }
}
...
// NOTE: This method relies on lock from calling function!
void IOInterfaceModbus::updateInputDiscreteModbusTransformAddress(
    const ConfigInputDiscrete &configInputDiscrete,
    std::vector<std::uint8_t> &dest8, std::vector<std::uint16_t> &dest16,
    std::vector<bool> &out) {
  switch (configInputDiscrete.functionCode) {
...
// NOTE: This method relies on lock from calling function!
void IOInterfaceModbus::updateInputDiscreteModbusTransformBitsLSB(
    const ConfigInputDiscrete &configInputDiscrete,
    std::vector<std::uint8_t> &dest8, std::vector<std::uint16_t> &dest16,
    std::vector<bool> &out) {
  switch (configInputDiscrete.functionCode) {
...
// NOTE: This method relies on lock from calling function!
void IOInterfaceModbus::updateInputDiscreteModbusTransformSpan2LSB(
    const ConfigInputDiscrete &configInputDiscrete,
    std::vector<std::uint8_t> &dest8, std::vector<std::uint16_t> &dest16,
    std::vector<bool> &out) {
  // For discrete we use only the first bit, so keep it simple and skip higher
  // bit addresses
  switch (configInputDiscrete.functionCode) {
  case FUNCTION_CODE_READ_BITS:
  case FUNCTION_CODE_READ_INPUT_BITS:
    for (std::size_t i = 0; i < configInputDiscrete.numAddressedItems / 2;
         i += 2) {
      out.push_back(static_cast<bool>(dest8[i]));
    }
    break;
...
// NOTE: This method relies on lock from calling function!
void IOInterfaceModbus::updateInputDiscrete(
    const ConfigInputDiscrete &configInputDiscrete) {
  if (modbus_set_slave(mCTX, configInputDiscrete.slaveID) == -1) {
    throw std::runtime_error("IOInterfaceModbus: Invalid slave ID: " +
                             std::to_string(configInputDiscrete.slaveID));
  }
  ...
  case ConfigFunctionsAddressLayout::ADDRESS:
    updateInputDiscreteModbusTransformAddress(configInputDiscrete, dest8,
                                              dest16, out);
    break;
  case ConfigFunctionsAddressLayout::BITS_LSB:
    updateInputDiscreteModbusTransformBitsLSB(configInputDiscrete, dest8,
                                              dest16, out);
    break;
  case ConfigFunctionsAddressLayout::SPAN_2_LSB:
    updateInputDiscreteModbusTransformSpan2LSB(configInputDiscrete, dest8,
                                               dest16, out);
    break;
  default:
    throw std::runtime_error(
        "IOInterfaceModbus: Incorrect discrete input address layout!");
    break;
  }
  std::shared_ptr<IOInterfaceInputDiscrete> values =
      getInputsDiscrete(configInputDiscrete.name);
  values->setValues(out);
}

IOInterfaceModbusPeriodicThread::IOInterfaceModbusPeriodicThread(
    std::string tcpAddress, std::uint16_t tcpPort,
...
void IOInterfaceModbusPeriodicThread::periodicAction() {
  try {
    ...
    BOOST_LOG_TRIVIAL(error)
        << "IOInterfaceModbusPeriodicThread: Refresh error: " << e.what();
    BOOST_LOG_TRIVIAL(error)
        << "IOInterfaceModbusPeriodicThread: Stopping due to error";
  } catch (...) {
    stop();
    mRefreshError = true;
    BOOST_LOG_TRIVIAL(error)
        << "IOInterfaceModbusPeriodicThread: Refresh error: Unknown";
    BOOST_LOG_TRIVIAL(error)
        << "IOInterfaceModbusPeriodicThread: Stopping due to error";
  }
  while (mRefreshError || !running() || !modbusInitialized()) {
    BOOST_LOG_TRIVIAL(info)
        << "IOInterfaceModbusPeriodicThread: Retrying connection";
    try {
      reconnect();
      start();
      mRefreshError = false;
      ...
      BOOST_LOG_TRIVIAL(error)
          << "IOInterfaceModbusPeriodicThread: Retrying connection error: "
          << e.what();
    } catch (...) {
      BOOST_LOG_TRIVIAL(error) << "IOInterfaceModbusPeriodicThread: Retrying "

...
void IOInterfaceModbus::initConnectionTCP(std::string tcpAddress,
    ...
    const std::unique_lock<std::shared_mutex> lock(mMutexModbusInitialize);
    mCTX =
        modbus_new_tcp_pi(tcpAddress.c_str(), std::to_string(tcpPort).c_str());
    if (mCTX == nullptr) {
        mModbusInitialized = false;
        throw std::runtime_error(
    ...
    BOOST_LOG_TRIVIAL(info)
        << "IOInterfaceModbus: Connected to " << tcpAddress << ":" << tcpPort;
}
...
void IOInterfaceModbus::refreshInputs() {
    ...
    if (mModbusInitialized) {
        ...
        // inputsDiscreteLock.unlock(); // will need this when other types of
        // input are added
    }
}
    ...
    if (configInputDiscrete.enableClear) {
        ...
        switch (configInputDiscrete.clearFunctionCode) {
            case FUNCTION_CODE_FORCE_SINGLE_COIL:
                res_clear = modbus_write_bit(
                    mCTX, configInputDiscrete.clearStartAddress, clear8_value);
                break;

            case FUNCTION_CODE_FORCE_MULTIPLE_COILS:
                res_clear = modbus_write_bits(
                    mCTX, configInputDiscrete.clearStartAddress, numClear,
                    &clear8[0]);
                break;
            ...
            default:
                throw std::runtime_error(
                    "IOInterfaceModbus: Incorrect discrete input clear "
                    "function code!");
                break;
        }
        if (res_clear == -1) {
...
void IOInterfaceModbus::updateInputDiscreteModbusTransformAddress(
    ...
    switch (configInputDiscrete.functionCode) {
        case FUNCTION_CODE_READ_BITS:
        case FUNCTION_CODE_READ_INPUT_BITS:
            for (std::size_t i = 0; i < configInputDiscrete.numAddressedItems;
                 i++) {
...
void IOInterfaceModbus::updateInputDiscreteModbusTransformBitsLSB(
    ...
    switch (configInputDiscrete.functionCode) {
        ...
        case FUNCTION_CODE_READ_INPUT_REGISTERS:
            for (std::size_t i = 0; i < configInputDiscrete.numAddressedItems;
                 i++) {
                for (std::size_t bit = 0; bit < MODBUS_BITS_PER_REGISTER;
                     bit++) {
...
void IOInterfaceModbus::updateInputDiscreteModbusTransformSpan2LSB(
    ...
    switch (configInputDiscrete.functionCode) {
        ...
        case FUNCTION_CODE_READ_INPUT_BITS:
            for (std::size_t i = 0;
                 i < configInputDiscrete.numAddressedItems / 2; i += 2) {
                out.push_back(static_cast<bool>(dest8[i]));
            }
            break;
        case FUNCTION_CODE_READ_REGISTERS:
...
void IOInterfaceModbus::updateInputDiscrete(
    ...
    switch (configInputDiscrete.addressLayout) {
        case ConfigFunctionsAddressLayout::ADDRESS:
            updateInputDiscreteModbusTransformAddress(configInputDiscrete,
                                                      dest8, dest16, out);
            break;
        case ConfigFunctionsAddressLayout::BITS_LSB:
            updateInputDiscreteModbusTransformBitsLSB(configInputDiscrete,
                                                      dest8, dest16, out);
            break;
        case ConfigFunctionsAddressLayout::SPAN_2_LSB:
            updateInputDiscreteModbusTransformSpan2LSB(configInputDiscrete,
                                                       dest8, dest16, out);
            break;
        default:
            throw std::runtime_error(
...
void IOInterfaceModbusPeriodicThread::periodicAction() {
    ...
    while (mRefreshError || !running() || !modbusInitialized()) {
        ...
        try {
            ...
            BOOST_LOG_TRIVIAL(error) << "IOInterfaceModbusPeriodicThread: "
                                        "Retrying connection error: "
                                     << e.what();
        } catch (...) {
            BOOST_LOG_TRIVIAL(error)
                << "IOInterfaceModbusPeriodicThread: Retrying "

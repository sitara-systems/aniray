   ...
   public:
    ...
    NodeArray(std::vector<NodeT> nodes,
        ...
        for (NodeT node : nodes) {
            ...
            for (auto const& [groupName, groupIndexes] : groups) {
                if (std::find(groupIndexes.begin(), groupIndexes.end(),
                              index) != groupIndexes.end()) {
        ...
        if (EVP_DigestInit_ex(m_context, EVP_get_digestbyname("sha512"),
                              nullptr) == 0) {
    ...
    NodeArray(
        std::tuple<std::vector<NodeT>,
                   std::map<std::string, std::vector<size_t>>> nodesAndGroups)
        : NodeArray(std::get<0>(nodesAndGroups), std::get<1>(nodesAndGroups)) {}
    // csv file
    NodeArray(const std::string& filename)
        : NodeArray{nodesFromCSV(filename)} {}
    ...
    NodeArray(NodeArray& targetNodeArray, bool useCache, double spacing)
        ...
        if (useCache) {
            std::string cacheName =
                getCacheName(spacing, targetNodeArray.mHash);
    ...
    void nodesToCSV(const std::string& filename) {
        ...
        for (std::shared_ptr<NodeT> node : mNodes) {
            ...
            file << "," << std::to_string(node->coords().x()) << ","
                 ...
                 << std::to_string(
                        node->coords()
                            .y());  // Real world z is up, in sim y is up
    ...
    template <typename onFindFunc>
    void findNodesInRadiusOfSource(Point sourceCoords,
                                   onFindFunc&& onFind,
                                   bool customRadius,
                                   double sampleRadius) {
        ...
        for (std::shared_ptr<NodeT> targetNode : mNodes) {
            ...
            if (std::abs(targetCoords.x() - sourceCoords.x()) >
                useSampleRadius) {
                continue;
            }
            if (std::abs(targetCoords.y() - sourceCoords.y()) >
                useSampleRadius) {
                continue;
            }
            if (std::abs(targetCoords.z() - sourceCoords.z()) >
                useSampleRadius) {
                continue;
            }
            if (comparableDistances.count(useSampleRadius) <= 0) {
                comparableDistances[useSampleRadius] =
                    boost::geometry::comparable_distance(
                        Point(targetCoords.x() - useSampleRadius,
                              targetCoords.y(), targetCoords.z()),
                        targetCoords);
            }
            double compare = 0;
            compare = comparableDistances[useSampleRadius];
            double distance = boost::geometry::comparable_distance(
   ...
   private:
    ...
    auto nodesFromCSV(const std::string& filename) const
        ...
        io::CSVReader<ANIRAY_NODES_FROM_CSV_NUM_COLUMNS,
            ...
            in(filename);
        in.read_header(io::ignore_extra_column, "universe", "startAddr", "x",
                       "y", "z", "xDeg", "yDeg", "zDeg", "ignore",
                       "sampleRadius", "groups", "inGrid", "gridIndexX",
                       "gridIndexY", "gridIndexZ");
        uint32_t universe = 0;
        uint8_t startAddr = 0;
        double x = 0;
        double y = 0;
        ...
        while (in.read_row(universe, startAddr, x, z, y, xDeg, zDeg, yDeg,
            ...
            if (ignoreStr == "true") {
                ...
                BOOST_LOG_TRIVIAL(warning)
                    << "NodeArray: Unknown ignore bool value for nodes {"
                    << addr << "}, ignoring nodes";
            }
            ...
            if (inGridStr == "true") {
                ...
                inGrid = false;
                BOOST_LOG_TRIVIAL(warning)
                    << "NodeArray: Unknown inGrid bool value for nodes {"
                    << addr << "}, ignoring nodes";
            }
            ...
            nodes.push_back(NodeT(
                Point(x, y, z * -1), Point(zDeg * -1, yDeg * -1, xDeg * -1),
                PointGridIndex(gridIndexX, gridIndexY, gridIndexZ), addr,
                ignore, sampleRadius));
            index++;
        }
        return std::make_tuple(nodes, groups);
    }
    auto nodeGridFromTargetSpacing(NodeArray& targetNodeArray,
                                   bool useCache,
        ...
        boost::geometry::strategy::transform::translate_transformer<double, 3,
                                                                    3>
            ...
            for (size_t j = 0; j < numY; j++) {
                for (size_t k = 0; k < numZ; k++) {
                    ...
                    targetNodeArray.findNodesInRadiusOfSource(
                        newPoint,
                        [newPoint, &nodes, i, j,
                         k]([[maybe_unused]] std::shared_ptr<NodeT>
                                targetNode) mutable -> bool {
                            nodes.push_back(NodeT(newPoint, Point(0, 0, 0),
                                                  PointGridIndex(i, j, k)));
                            return true;
                        });
                }
            }
